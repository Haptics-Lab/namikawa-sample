...
logging_queue = Queue()
file_queue = Queue(maxsize=200000)
sync_ready = threading.Event()
raw_queue = Queue(maxsize=40000) 
...
EARLY_REQUEST_MARGIN_CHUNKS = 10
NEXT_REQUEST_STEP = CHUNK_NUM
    ...
    with open(BIN_FILE, "wb") as f:
        while not log_done.is_set() or not file_queue.empty():
            try:
                ...
                packed = struct.pack(f"<B I {CHUNK_SIZE}f", ch, index, *values)
                f.write(packed)
...
def main():
    ...
    try:
        ...
        # --- 事前に全CHぶんの送信コマンドを作成 ---
        def make_request_cmds(chunk_num):
            return ''.join(
                f"*40{ch:X}1{chunk_num - 1:04X}#"
                for ch in range(REQUEST_DATA_NUM)
            )

        cmds_first = make_request_cmds(CHUNK_NUM)
        cmds_repeat = make_request_cmds(CHUNK_NUM + EARLY_REQUEST_MARGIN_CHUNKS)

        print("一括送信コマンド(初回):", cmds_first)
        print("一括送信コマンド(繰り返し):", cmds_repeat)
        ...
        # ==== 送信要求コマンド送信 ====
        print("送信要求コマンド送信(1回目)")
        handle.write(cmds_first.encode())

        request_count = 1
        next_trigger = CHUNK_NUM - EARLY_REQUEST_MARGIN_CHUNKS
        stop_reason = "Ctrl+C"

        print("[MAIN] Ctrl+Cで終了するまで、受信チャンク数基準で送信要求を繰り返します")
        print(f"[MAIN] 初回は全CHが {next_trigger} チャンク受信したら次回要求を送信します")

        try:
            while running:
                min_received = min(recv_chunk_index[ch] for ch in range(REQUEST_DATA_NUM))

                if min_received >= next_trigger:
                    request_count += 1
                    print(
                        f"送信要求コマンド送信({request_count}回目) "
                        f"min_received={min_received}, next_trigger={next_trigger}"
                    )
                    handle.write(cmds_repeat.encode())

                    # 次の送信タイミング
                    next_trigger += CHUNK_NUM
        ...
        for ch in range(REQUEST_DATA_NUM):
            ...
            status = "OK" if (saved == received) else "NG"

            print(f"CH{ch}: {status}")
            print(f"  受信: {received}")
            print(f"  保存: {saved}  (未保存: {miss_save})")
...
def load_saved_records(bin_file):
    ...
    rec_size = 1 + 4 + 4 * CHUNK_SIZE  # uint8 + uint16 + float32 * CHUNK_SIZE
    ...
    with open(bin_file, "rb") as f:
        for _ in range(total_recs):
            ...
            if len(data) != rec_size:
                break
            ch, idx = struct.unpack("<B I", data[:5])
            values = struct.unpack(f"<{CHUNK_SIZE}f", data[5:])
